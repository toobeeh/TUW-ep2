...
public class Simulation9 {
    ...
    public static void main(String[] args) {
        ...
        HashMap<NamedBody, Vector3> forceMap = new HashMap<>();
        ...
        applyConfig(forceMap, earth, args[0], args[1]);
        applyConfig(forceMap, moon, args[0], args[1]);
        applyConfig(forceMap, venus, args[0], args[1]);
        applyConfig(forceMap, mercury, args[0], args[1]);
        applyConfig(forceMap, mars, args[0], args[1]);

        // add sun after states have been read from files.
        forceMap.put(new NamedBody("Sun", 1.989E30, new Vector3(0, 0, 0), new Vector3(0, 0, 0)),
                new Vector3(0, 0, 0));
        ...
        for(int seconds = 0; true; seconds++){
            ...
            // calculate forces
            HashMap<NamedBody, Vector3> massiveMap = new HashMap<>();

            for(NamedBody massive : forceMap.keySet()){
                Vector3 force = new Vector3(0,0,0);

                for(NamedBody forceOf : forceMap.keySet()){
                    if(!forceOf.equals(massive))
                        force = force.plus(massive.gravitationalForce(forceOf));
                }
            ...
            // move massives
            for(Massive massive : massiveMap.keySet()){
    ...
    static void applyConfig(HashMap<NamedBody, Vector3> map, NamedBody body, String path, String day){
        try{
            ...
            System.out.println("Unknown IO exception occured for body: " + body.name);
            map.remove(body);
        }
    }
}
